...
def sync_clients(payload: dict):
    ...
    for c in clients:
        ...
        for old_row in old_rows:
            ...
            attendance_rows = cur.execute("""
                SELECT attended_date, present, finalized
                FROM attendance
                WHERE client_id = ?
            """, (old_id,)).fetchall()

            for attended_date, present, finalized in attendance_rows:
                cur.execute("""
                    INSERT OR IGNORE INTO attendance
                        (client_id, attended_date, present, finalized)
                    VALUES (?, ?, ?, ?)
                """, (incoming_id, attended_date, present, finalized))

            cur.execute("DELETE FROM attendance WHERE client_id = ?", (old_id,))
